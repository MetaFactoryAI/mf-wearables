...
if [ -z "$1" ] || [ -z "$2" ]; then
  echo "Usage: $0 <HDR_FILE> <SKYBOX_DIR>"
  exit 1
fi
...
# Loop through all skybox files in the skybox directory
for skybox_file in "${skybox_dir}"/; do
...
<head>
  ...
  <script type="module" src="https://ajax.googleapis.com/ajax/libs/model-viewer/3.0.1/model-viewer.min.js%22%3E</script>
  <style>
    html, body {
      margin: 0;
      height: 100%;
