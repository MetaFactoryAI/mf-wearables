...
# Check if HDR file and skybox files are provided
#if [ -z "$1" ] || [ -z "$2" ]; then
#  echo "Usage: $0 <HDR_FILE> <SKYBOX_FILE>"
#  exit 1
#fi
...
<head>
  ...
  <script>
    ...
    function changeSkybox() {
      ...
      modelViewer.setAttribute('skybox-image', "../../$currentImage.jpg");
    }
  </script>
</head>
<body>
  <div class="buttons">
    ...
    <button onclick="document.querySelector('model-viewer').changeSkybox()">Change Skybox</button>
  </div>
  <model-viewer src="${glb}" camera-controls auto-rotate touch-action="pan-y" skybox-image='../../${currentImage}.jpg' environment-image="../../${hdr_file}"></model-viewer>
</body>
</html>
EOL
